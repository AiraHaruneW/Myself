...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Check if it's mobile (below 768px screen width)
    const isMobile = window.innerWidth <= 768;

    function highlightCenterRow() {
        if (!isMobile) return; // Do nothing if it's not a mobile screen
    ...
    window.addEventListener("scroll", highlightCenterRow);
    window.addEventListener("resize", () => {
        // Update the check for mobile screen size on resize
        isMobile = window.innerWidth <= 768;
        highlightCenterRow();
    });
    highlightCenterRow(); // initial run
});
